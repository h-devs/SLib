...
void P2PMsgApp::onStart()
{
	...
	window->setOnOK([this](Window* window) {
		...
		m_p2p->sendMessage(nodeId, msg, [this, weakWindow, name, msg](P2PResponse& response) {
			auto window = ToRef(weakWindow);
			if (window.isNull() || window->isClosed()) {
				return;
			}
